...
public abstract class PowerContainerVmAllocationPolicyMigrationAbstract extends PowerContainerVmAllocationAbstract {
    ...
    public List<Map<String, Object>> optimizeAllocation(List<? extends ContainerVm> vmList) {
        ExecutionTimeMeasurer.start("optimizeAllocationTotal");//Startzeitpunkte festlegen

        ExecutionTimeMeasurer.start("optimizeAllocationHostSelection");//Startzeitpunkte festlegen für hosts
        List<PowerContainerHostUtilizationHistory> overUtilizedHosts = getOverUtilizedHosts();//Liste mit overutilized hosts
        getExecutionTimeHistoryHostSelection().add(
                ExecutionTimeMeasurer.end("optimizeAllocationHostSelection"));//Endzeitpunkt für hostselection

        printOverUtilizedHosts(overUtilizedHosts);//log für overutilized hosts

        saveAllocation();//für jede vm wird der zugehörige host und die vm gemappt und übergeben

        ExecutionTimeMeasurer.start("optimizeAllocationVmSelection");
        List<? extends ContainerVm> vmsToMigrate = getVmsToMigrateFromHosts(overUtilizedHosts);//VMs werden von den hosts entfernt, bis diese nicht mehr overutilized sind.
        //Dabei werden die VMs in einer map gespeichert, um später migiert werden zu können
        getExecutionTimeHistoryVmSelection().add(ExecutionTimeMeasurer.end("optimizeAllocationVmSelection"));//Ende der selection

        Log.printLine("Reallocation of VMs from the over-utilized hosts:");
        ExecutionTimeMeasurer.start("optimizeAllocationVmReallocation");//Startzeitpunkt verteilung der zu migrierenden vms
